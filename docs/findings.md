...
| bf16 KDA recurrent state (`--mamba-ssm-cache-dtype bfloat16`, needs patch 09) | KDA kernel 2x faster (722 vs 1,418 us at 32 sequences) | C8 +1.7%, C32 +1.7%, quality unchanged | No |
...
- **Prefix caching works in 2,304-token blocks on this hybrid model.** vLLM caches the KDA layers' state only at block boundaries (mamba cache mode `align`, KV block size 2,304), so a prompt shorter than one block is recomputed on every turn: 264K prompt tokens of short agent traffic got zero cache hits. Long prompts are cached one full block at a time, which is where the faster follow-up turns come from.
